...
Повторный запуск **`./install.sh`**: существующий **`.env` не перезаписывается** (вопросы мастера пропускаются). Чтобы снова пройти мастер: удалите `.env` или выполните **`TG_JOIN_GIF_FORCE_RECONFIGURE=1 ./install.sh`**. Каталог **`.venv`** при повторе не пересоздаётся с нуля — только обновляются зависимости из `requirements.txt`.
...
- **`/start`** — если задана приветственная гифка, сначала она, затем краткое описание и сводка настроек.
- **`/settings`** — та же сводка настроек (без гифки).
...
Это **не ожидаемая** нормальная работа: у одного бота может быть **только один** активный long polling на `getUpdates`. Сообщение значит, что с **тем же `BOT_TOKEN`** уже запущен другой процесс (или включён webhook).

Частые причины:

- одновременно **systemd** и ручной запуск `python bot.py`;
- две ВМ / два контейнера с **одинаковым** `.env`;
- для бота настроен **webhook**, а вы ещё запустили polling.

Что сделать: оставить **один** экземпляр (`systemctl stop …` для лишнего, проверить `ps aux | grep bot.py`), при webhook — выключить его (`deleteWebhook` или смена режима в коде/инфраструктуре).
